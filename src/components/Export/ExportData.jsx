  ...
  const handleExportTxt = (fileName) => {
    try {
      const txtContent = data
        .map((item) => columns.map((column) => getColumnValue(item, column) || '').join(' '))
        .join('\n');
        ...
        .map(
          (item) =>
            ...
            columns
              .map(
                (column) =>
                  `<${column.header.replace(/ /g, '')}>${getColumnValue(item, column)}</${column.header.replace(/ /g, '')}>`
              )
              .join('') +
            '</item>'
        )
  ...
  const renderExportProgress = () => {
    ...
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', borderRadius: 1, mt: 2 }}>
        <Tooltip
          title={isComplete ? `${exportType.toUpperCase()} Export Complete` : `Exporting ${exportType.toUpperCase()}`}
        >
    ...
    <Box align="right" style={style} className={className}>
      {componentVariant === 'ButtonGroup' ? (
        <ButtonGroup size="small" aria-label="Small button group" onClick={customOnClick} {...others}>
          {exportOptions
            .filter((option) => exportTypes.includes(option.fileExtension))
            .map((option) => (
              <Button
                key={option.label}
        ...
        <>
          <Button
            disabled={isLoading}
            aria-controls={anchorElement ? 'export-menu' : undefined}
            aria-haspopup="true"
          ...
          >
            <Stack sx={{ '& .MuiMenuItem-root+.MuiDivider-root': { my: '0.25rem' } }} divider={<Divider />}>
              {exportOptions
                .filter((option) => exportTypes.includes(option.fileExtension))
                .map((option) => (
                  <MenuItem key={option.label} onClick={() => handleExport(option.fileExtension)} disabled={isLoading}>
...
ExportData.propTypes = {
  ...
  useApiForExport: PropTypes.bool,
