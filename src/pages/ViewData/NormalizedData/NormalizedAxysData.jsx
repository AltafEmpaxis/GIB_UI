...
import { NormalizedAxysDataColumns } from 'components/Table/Columns/Columns';
import ReusableTable from 'components/Table/ReusableTable';
import useAuth from 'hooks/useAuth';
import axios from 'utils/axios';

const NormalizedAxysData = () => {
  const { user } = useAuth();
  const columns = NormalizedAxysDataColumns();
  ...
  const fetchNormalizedAxysData = useCallback(async () => {
    try {
      const response = await axios.get('/api/NormaliseFile/GetNormalizedAxysData/normalized-axys-data', {
        params: {
          userId: user?.user_id,
      ...
      console.error('Error fetching normalized Custodian data:', err);
      throw err;
    }
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="NormalizedAxysData"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
  ...
  return (
    <>
      <title>Custodian Normalized Data</title>
      <meta name="description" content="View and analyze normalized tax lots data from Custodian system" />
      <meta property="og:title" content="Custodian Normalized Data" />
      <meta property="og:description" content="View and analyze normalized tax lots data from Custodian system" />
...
export default NormalizedAxysData;
