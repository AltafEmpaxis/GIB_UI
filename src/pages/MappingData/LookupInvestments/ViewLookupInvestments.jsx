...
export default function ViewLookupInvestments() {
  ...
  const { data, isLoading, error, isError, isPending } = useQuery({
    queryKey: ['Page>MappingData>Tab>ViewLookupInvestments', pagination.pageIndex, pagination.pageSize],
    queryFn: async () => {
      try {
        const response = await axios.get(endpoints.lookupInvestments, {
          params: {
        ...
        console.error('Error fetching View Lookup Investments:', error);
        throw error;
      }
    },
  ...
  const { mutateAsync: createMapping } = useMutation({
    mutationFn: async (values) => {
      const response = await axios.post(endpoints.lookupInvestments, values);
      return response.data;
    },
    onMutate: async () => {
      await queryClient.cancelQueries(['Page>MappingData>Tab>ViewLookupInvestments']);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupInvestments']);
      Swal.fire({
        icon: 'success',
        title: 'Success',
  ...
  const { mutateAsync: updateMapping } = useMutation({
    mutationFn: async (values) => {
      const response = await axios.put(`${endpoints.lookupInvestments}/${values.id}`, values);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupInvestments']);
  ...
  const { mutateAsync: deleteMapping } = useMutation({
    mutationFn: async (id) => {
      await axios.delete(`${endpoints.lookupInvestments}/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupInvestments']);
      Swal.fire({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="View_LookupInvestments"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Lookup Investments"
        />
        <Button
          variant="contained"
          size="small"
