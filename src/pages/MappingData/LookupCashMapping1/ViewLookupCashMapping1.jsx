...
export default function ViewLookupCashMapping1() {
  ...
  const { data, isLoading, error, isError, isPending } = useQuery({
    queryKey: ['Page>MappingData>Tab>LookupCashMapping1'],
    queryFn: async () => {
      try {
        const response = await axios.get(endpoints.getLookupCashMapping1);
        return response.data;
      } catch (error) {
        console.error('Error fetching View Lookup CashMapping1:', error);
        throw error;
      }
    },
  ...
  const { mutateAsync: createMapping } = useMutation({
    mutationFn: async (values) => {
      ...
      const response = await axios.post(endpoints.createLookupCashMapping1, values);
      return response.data;
    },
    onMutate: async () => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries(['Page>MappingData>Tab>LookupCashMapping1']);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>LookupCashMapping1']);
      Swal.fire({
        icon: 'success',
        title: 'Success',
  ...
  const { mutateAsync: updateMapping } = useMutation({
    mutationFn: async (values) => {
      const response = await axios.put(`${endpoints.updateLookupCashMapping1}/${values.id}`, values);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>LookupCashMapping1']);
  ...
  const { mutateAsync: deleteMapping } = useMutation({
    mutationFn: async (id) => {
      await axios.delete(`${endpoints.deleteLookupCashMapping1}/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>LookupCashMapping1']);
      Swal.fire({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="View_LookupCashMapping1"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Lookup Cash Mapping1"
        />
        <Button
          variant="contained"
          size="small"
