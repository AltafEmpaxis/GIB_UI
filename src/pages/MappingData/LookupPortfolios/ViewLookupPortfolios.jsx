...
import { endpoints, fetchData, createRecord, updateRecord, deleteRecord } from 'utils/axios';
...
export default function ViewLookupPortfolios() {
  ...
  const { data, isLoading, error, isError, isPending } = useQuery({
    queryKey: ['Page>MappingData>Tab>ViewLookupPortfolios', pagination.pageIndex, pagination.pageSize],
    queryFn: () =>
      fetchData([
        endpoints.lookupPortfolios,
        {
          params: {
  ...
  const { mutateAsync: createMapping } = useMutation({
    mutationFn: (values) => createRecord(endpoints.lookupPortfolios, values),
    onMutate: async () => {
      await queryClient.cancelQueries(['Page>MappingData>Tab>ViewLookupPortfolios']);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupPortfolios']);
      Swal.fire({
        icon: 'success',
        title: 'Success',
  ...
  const { mutateAsync: updateMapping } = useMutation({
    mutationFn: (values) => updateRecord(endpoints.lookupPortfolios, values.id, values),
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupPortfolios']);
      Swal.fire({
        icon: 'success',
  ...
  const { mutateAsync: deleteMapping } = useMutation({
    mutationFn: (id) => deleteRecord(endpoints.lookupPortfolios, id),
    onSuccess: () => {
      queryClient.invalidateQueries(['Page>MappingData>Tab>ViewLookupPortfolios']);
      Swal.fire({
        icon: 'success',
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="View_LookupPortfolios"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Lookup Portfolios"
        />
        <Button
          variant="contained"
          size="small"
