...
import { SkippedColumns } from 'components/Table/Columns/Columns';
import useAuth from 'hooks/useAuth';

const SkippedReport = () => {
  const { user } = useAuth();
  const columns = SkippedColumns();
  ...
  const fetchSkippedData = useCallback(async () => {
    try {
      const response = await axios.get(endpoints.generateReconTable, {
        params: {
          userId: user?.user_id,
          reconciliationType: 'Skipped',
      ...
      console.error('Error fetching skipped report:', err);
      throw err;
    }
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="Skipped_Report"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Skipped Report"
        />
      </Box>
    ),
