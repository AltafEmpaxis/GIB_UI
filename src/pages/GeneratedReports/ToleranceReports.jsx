...
import { ToleranceColumns } from 'components/Table/Columns/Columns';
import useAuth from 'hooks/useAuth';

const ToleranceReports = () => {
  const { user } = useAuth();
  const columns = ToleranceColumns();
  ...
  const fetchToleranceData = useCallback(async () => {
    try {
      ...
      console.error('Error fetching tolerance report:', err);
      throw err;
    }
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="Tolerance_Report"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Tolerance Report"
        />
      </Box>
    ),
...
export default ToleranceReports;
