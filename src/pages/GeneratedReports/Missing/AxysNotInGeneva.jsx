...
import { AxysNotInGeneva } from 'components/Table/Columns/Columns';
...
const AxysTaxLots = () => {
  ...
  const fetchAxysNotInGenevaData = useCallback(async () => {
    try {
      const response = await axios.get(endpoints.generateReconTable, {
        params: {
          userId: user?.user_id,
          reconciliationType: 'AxysNotInGeneva',
      ...
      console.error('Error fetching missing Custodian report:', err);
      throw err;
    }
  ...
  const { data, isLoading, error, isError, isPending } = useQuery({
    ...
    queryFn: fetchAxysNotInGenevaData,
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="Missing_Axys_Report"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Missing Custodian Report"
        />
      </Box>
    ),
