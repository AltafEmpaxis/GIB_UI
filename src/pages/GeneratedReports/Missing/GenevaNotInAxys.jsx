...
import { GenevaNotInAxys } from 'components/Table/Columns/Columns';
import ReusableTable from 'components/Table/ReusableTable';
import useAuth from 'hooks/useAuth';
import axios, { endpoints } from 'utils/axios';

const GenevaNotInAxyss = () => {
  const { user } = useAuth();
  const columns = GenevaNotInAxys();
  ...
  const fetchGenevaNotInAxysData = useCallback(async () => {
    try {
      const response = await axios.get(endpoints.generateReconTable, {
        params: {
          userId: user?.user_id,
          reconciliationType: 'GenevaNotInAxys',
      ...
      console.error('Error fetching missing APX report:', err);
      throw err;
    }
  ...
  const { data, isLoading, error, isError, isPending } = useQuery({
    ...
    queryFn: fetchGenevaNotInAxysData,
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
        <ExportData
          ...
          ExportFileName="Missing_Geneva_Report"
          isLoading={isLoading}
          variant="contained"
          color="primary"
          size="small"
          componentVariant="Menu"
          aria-label="Export Missing APX Report"
        />
      </Box>
    ),
...
export default GenevaNotInAxyss;
